        ...
        <a-list-item-meta>
          ...
          <template slot="description">
            <div class="description">
              ...
              <span class="pub-date">{{ item.created | datetimeFormat }}</span>
              <span class="tag"><a-tag v-for="(cat, index) in item.category" v-bind:key="index">
                <a :href="cat.scheme" target="_blank">{{ cat.term }}</a>
              </a-tag></span>
            </div>
          </template>
        </a-list-item-meta>
...
@Component({
  ...
  filters: {
    ...
    datetimeFormat(content: number) {
      let dateTypeDate = '';
      const date = new Date();
      date.setTime(content);
      dateTypeDate += date.getFullYear();
      dateTypeDate += '-' + date.getMonth();
